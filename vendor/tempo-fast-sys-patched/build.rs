...
//!
//! # Jimmy-side patch (not part of upstream Nexus)
//! This is a local copy of Nexus's own crates/tempo-fast-sys, kept ONLY because this
//! environment's `std::path::Path::canonicalize()` returns a `\\?\`-prefixed extended-length
//! path, and this environment's gfortran/ninja/cmake combination mishandles that prefix during
//! Fortran preprocessor dependency-file path reconstruction (observed failure:
//! `f951.exe: Fatal Error: //<file>.f90: No such file or directory` -- the drive letter is lost
//! entirely, not just the `\\?\` marker). Two changes from upstream, both confined to this file:
//! (1) `strip_verbatim_prefix` below, applied everywhere a canonicalized path is turned into a
//! string handed to an external tool; (2) `libtempo_src` now resolves via `NEXUS_LIBTEMPO_DIR`
//! (defaulting to the real Nexus checkout this was copied from) instead of a `../../libtempo`
//! relative path, since this crate no longer lives two directories above it. The GPL-3.0-licensed
//! Fortran/C modem source under libtempo/ is NOT copied here and is not vendored into Jimmy's
//! repo -- this crate still builds the real Nexus libtempo/ tree in place, only from a different
//! Cargo source location. See EngineHost/Cargo.toml's [patch] section for how this is wired in.
...
/// Strip the `\\?\` extended-length-path prefix Windows' `canonicalize()` adds, matching what
/// the `dunce` crate does -- external tools (gfortran, ninja, cmake's Fortran depfile handling)
/// in this environment don't round-trip that prefix correctly.
...
fn main() {
    ...
    let _ = manifest; // upstream used this to locate a sibling libtempo/; unused on this path.
    let libtempo_src_raw = PathBuf::from(
        env::var("NEXUS_LIBTEMPO_DIR").unwrap_or_else(|_| "C:/claude/nexus/libtempo".to_string()),
    );
    let libtempo_src = strip_verbatim_prefix(
        &libtempo_src_raw
            .canonicalize()
            .expect("locate Nexus libtempo (set NEXUS_LIBTEMPO_DIR if not at C:/claude/nexus)"),
    );
...
fn emit_rerun_glob(p: &std::path::Path) {
    let canon = strip_verbatim_prefix(&p.canonicalize().unwrap_or_else(|e| {
        panic!(
