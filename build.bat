...
rem -p:AssemblyName="Jimmy Next" -- 2026-08-10: local Debug builds used to compile as plain
...
rem Setup_WiX\JimmyNext.wxs already builds the separate JimmyNext.msi product (same
...
dotnet build "%~dp0WSJTX_Controller\Jimmy.csproj" -c Debug -p:AssemblyName="Jimmy Next" -v:minimal > "%~dp0build_out.txt" 2>&1
